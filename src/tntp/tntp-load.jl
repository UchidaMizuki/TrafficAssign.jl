...
function load_tntp_trips(file_trips)
  open(file_trips, "r") do file
    ...
    # @assert size(unique([orig; dest]), 1) == n_zones
    # @assert isapprox(sum(trips), total_od_flow)
...
function load_tntp_network(file_network)
  open(file_network, "r") do file
    ...
    # @assert issetequal([from; to], 1:n_nodes)
    # @assert idx == n_edges
